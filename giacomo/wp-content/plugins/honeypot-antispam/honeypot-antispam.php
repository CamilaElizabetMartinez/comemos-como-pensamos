...
define( 'ANTISPAM_PLUGIN_VERSION', '1.0.4' );
...
function antispam_enqueue_script() {
	...
	if ( ( is_singular() || $withcomments ) && comments_open() ) { // load script only for pages with comments form
		wp_enqueue_script( 'honeypot-antispam-script', plugins_url( '/js/honeypot-antispam.js', __FILE__ ), array(), ANTISPAM_PLUGIN_VERSION, true );
	}
}
...
function antispam_form_part() {
	...
	if ( ! is_user_logged_in() ) { // add honeypot-antispam fields only for not logged in users
...
function antispam_check_comment( $commentdata ) {
	...
	if ( ! is_user_logged_in() && $comment_type !== 'pingback' && $comment_type !== 'trackback' && $comment_type !== 'webmention' ) { // logged in user is not a spammer
